...
//	@Override
//	public BillDto generateBill(BillDto billDto,Integer userId) {
//		
...
//		
//		for(Iteam i:items) {
//			Integer price = i.getSellingPrice();
//			total=total+price;
//		}
//		
//		
...
//		
//		
//		return this.mapper.map(saveBill, BillDto.class);
//	}
	@Override
	public BillDto generateBill(BillDto billDto, Integer userId) throws Exception {
	    ...
	    Cart cart = user.getUserCart();
	    if (cart == null) {
	        // Handle the case where the user has no cart (you may throw an exception, log a message, etc.)
	        throw new Exception("User has no cart");
	    }
	  List<Iteam> items = cart.getIteams();
	  System.out.println(items+"!!!!!!!!!!");
	  if (items == null) {
	        // Handle the case where the cart has no items (you may throw an exception, log a message, etc.)
	...
	private void updateUserInformation(User user, BillDto billDto) {
	    ...
	    user.setName(billDto.getName());
	    user.setPhoneNo(billDto.getPhoneNo());
//	   user.setUserId(billDto.getUserId());
	}
	...
	private void setBillDtoProperties(BillDto billDto, Cart cart, Integer total) {
	    ...
	    billDto.setTotalAmount(total);
	    billDto.setCartId(cart.getCartId());
	    billDto.setAllItems(cart.getIteams());
